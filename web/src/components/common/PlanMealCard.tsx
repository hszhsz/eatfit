    ...
    <article className="rounded-3xl border border-[#F0E6DD] bg-white p-5 shadow-warm">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="rounded-full border border-[#F0E6DD] px-3 py-1 text-xs text-[#6B5544]">
          {formatNumber(meal.recipe.calories, 0, lang)} kcal
        </div>
      </div>

      <div className="mt-4 flex items-center gap-2 text-sm text-[#6B5544]">
        <Clock3 className="h-4 w-4" />
        {t("recipes.cookTime", { min: meal.recipe.cookMinutes })}
      </div>

      <div className="mt-5 grid grid-cols-3 gap-3 text-sm">
        <div className="rounded-2xl bg-[#FFF5EE] px-3 py-3 text-[#6B5544]">
          <div className="text-[#9C8B7A]">{t("plan.protein")}</div>
          <div className="mt-1 font-medium text-[#1F1611]">
            {formatNumber(meal.recipe.proteinG, 1, lang)} g
          </div>
        </div>
        <div className="rounded-2xl bg-[#FFF5EE] px-3 py-3 text-[#6B5544]">
          <div className="text-[#9C8B7A]">{t("plan.carbs")}</div>
          <div className="mt-1 font-medium text-[#1F1611]">
            {formatNumber(meal.recipe.carbsG, 1, lang)} g
          </div>
        </div>
        <div className="rounded-2xl bg-[#FFF5EE] px-3 py-3 text-[#6B5544]">
          <div className="text-[#9C8B7A]">{t("plan.fat")}</div>
          <div className="mt-1 font-medium text-[#1F1611]">
            {formatNumber(meal.recipe.fatG, 1, lang)} g
          </div>
        </div>
      </div>
